...
fn generate_for_statement(
    ...
    match s {
        ...
        ast::Statement::WhileLoop(cond, body) => {
            ...
            let cond_ir = generate_for_predicate(cond, body_label.clone(), cont_label, blocks, env);
            let cond_block = blocks.get_mut(&cond_label).unwrap();
            cond_block.statements = cond_ir;

            vec![ir::Statement::Goto(cond_label)]
        }
        ast::Statement::Return(expr) => generate_for_tail(expr, blocks, env),
    }
}
...
fn generate_for_tail(e: ast::Expr, blocks: &mut BlockMap, env: &TypeEnv) -> Vec<ir::Statement> {
    match e {
        ...
        _ => {
            ...
            let size = e.get_type(env).size();
            generate_for_assign(
                e,
                AssignDest::Id(ret_var.clone()),
                vec![ir::Statement::Return(ir::Atom::new_variable(ret_var, size))],
                blocks,
                env,
            )
        }
...
mod tests {
    ...
    use test_support::{
        ...
        compiler::{
            ...
            passes::{
                ASTPass, ASTtoIRPass, GlobalizeIdentifiers, RemoveComplexOperands, ShortCircuiting,
                TranslateASTtoIR, TypeCheck,
            },
    ...
    fn execute_test_case(mut tc: TestCase) {
        ...
        let globalized = GlobalizeIdentifiers.run_pass(post_rco_ast);
        let type_checked2 = TypeCheck.run_pass(globalized);

        println!("-- AST before ASTtoIR:\n{type_checked2:?}");
        let post_translation = TranslateASTtoIR.run_pass(type_checked2);
    ...
    fn test_condition_statement() {
        execute_test_case(TestCase {
            ...
            inputs: VecDeque::from(vec![Value::I64(1), Value::I64(2)]),
    ...
    fn test_while_loop_accumulator() {
        ...
        execute_test_case(TestCase {
            ...
            inputs: VecDeque::new(),
            expected_outputs: VecDeque::from(vec![Value::I64(15)]), // sum of 1..5
        });
    }
                ...
                functions: vec![Function {
                    ...
                    body: vec![
                        ...
                        Statement::WhileLoop(
                            ...
                            vec![
                                Statement::Assign(
                                    AssignDest::Id(t_global!("j")),
                                    Expr::Constant(Value::I64(0)),
                                    None,
                                ),
                                Statement::WhileLoop(
                                    Expr::BinaryOp(
                                        Box::new(Expr::Id(t_global!("j"))),
                                        BinaryOperator::Less,
                                        Box::new(Expr::Constant(Value::I64(2))),
                                    ),
                                    vec![
                                        Statement::Expr(Expr::Call(
                                            Box::new(Expr::GlobalSymbol(t_global!("print_int"))),
                                            vec![Expr::Id(t_global!("i"))],
                                        )),
                                        Statement::Expr(Expr::Call(
                                            Box::new(Expr::GlobalSymbol(t_global!("print_int"))),
                                            vec![Expr::Id(t_global!("j"))],
                                        )),
                                        Statement::Assign(
                                            AssignDest::Id(t_global!("j")),
                                            Expr::BinaryOp(
    ...
    fn test_apply_with_different_functions() {
        ...
        execute_test_case(TestCase {
            ...
            inputs: VecDeque::new(),
            expected_outputs: VecDeque::from(vec![Value::I64(20), Value::I64(11)]),
        });
    }
}
