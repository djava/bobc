...
mod tests {
    ...
    fn check_invariants(p: &x86::X86Program) {
            ...
            .flatten()
        {
            ...
            match i {
                ...
                Instr::cmp(_, d) if matches!(d.value, ArgValue::Immediate(_)) => {
                    panic!("PatchInstructions should remove all cmpq's with Imm d");
                }
                Instr::imul(_, d) if !matches!(d.value, ArgValue::Reg(_)) => {
    ...
    fn test_multiply_with_spilled_dest() {
        ...
        let mut body: Vec<Statement> = (1..=15)
            .map(|i| {
                Statement::Assign(
                    AssignDest::Id(t_global!(format!("v{i}").as_str())),
        ...
        body.push(Statement::Assign(
            AssignDest::Id(t_global!("result")),
            Expr::BinaryOp(
                Box::new(Expr::Id(t_global!("v1"))),
                BinaryOperator::Multiply,
                Box::new(Expr::Id(t_global!("v2"))),
            ),
            None,
        ));

        // Use all variables to keep them alive across the multiply
        let mut sum_expr: Expr = Expr::Id(t_global!("v1"));
        for i in 2..=15 {
            sum_expr = Expr::BinaryOp(
                Box::new(sum_expr),
                BinaryOperator::Add,
                Box::new(Expr::Id(t_global!(format!("v{i}").as_str()))),
            );
        }
        sum_expr = Expr::BinaryOp(
            Box::new(sum_expr),
            BinaryOperator::Add,
            Box::new(Expr::Id(t_global!("result"))),
        );
